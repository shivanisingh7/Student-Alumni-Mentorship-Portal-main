...
const UpcomingEvents = () => {
  ...
  const fetchStudentNameById = async (studentId) => {
    try {
      const response = await axios.get(
        ...
        `${window.location.origin}/api/v1/filter-student/getStudentNameById?studentId=${studentId}`
      );
  ...
  async function showUpcomingEvents() {
    ...
    try {
      ...
      const baseUrl = `${window.location.origin}/api/v1/fetchSlots/details`;
  ...
  async function deleteEvent(eventId, googleEventId) {
    try {
      ...
      const deleteResponse = await axios.delete(
        ...
        `${window.location.origin}/api/v1/deleteEvent/details?eventId=${eventId}`
      );
