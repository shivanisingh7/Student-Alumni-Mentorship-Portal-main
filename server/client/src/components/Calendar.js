...
const Calendar = () => {
  ...
  const fetchAlumniNameById = async (alumniId) => {
    try {
      const response = await axios.get(
        `${window.location.origin}/api/v1/student/filter-alumni/getAlumniNameById?alumniId=${alumniId}`//`http://localhost:4000/api/v1/student/filter-alumni/getAlumniNameById?alumniId=${alumniId}`
      );
  ...
  const handleShowPastMeetings = async () => {
    ...
    try {
      const response = await axios.get(
        `${window.location.origin}/api/v1/fetchPastMeetings/meetings?userId=${user._id}`// `http://localhost:4000/api/v1/fetchPastMeetings/meetings?userId=${user._id}`
      );
  ...
  async function createCalendarEvent() {
    ...
    try {
      ...
      const response = await axios.post(
         `${window.location.origin}/api/v1/saveEvent/details`,// "http://localhost:4000/api/v1/saveEvent/details",
        {
          headers: {
  ...
  async function syncCalendar() {
    if (session !== null && session?.provider_token !== undefined) {
      try {
        const baseUrl = `${window.location.origin}/api/v1/fetchSlots/details`;//"http://localhost:4000/api/v1/fetchSlots/details";
              ...
              if (
                ...
                try {
                  ...
                  if (response.ok) {
                    ...
                    const apiUrl =
                      `${window.location.origin}/api/v1/updateEvent/update`;//"http://localhost:4000/api/v1/updateEvent/update";
                    axios
                      .post(apiUrl, {
                        headers: {
                          "Content-Type": "application/json",
